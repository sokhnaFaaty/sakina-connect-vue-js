...
<template>
  ...
  <section v-else>
    ...
    <div class="mb-6 grid gap-4 sm:grid-cols-2">
      ...
      <div class="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div class="mt-3 flex items-center justify-between rounded-xl bg-[#F2F2DE] p-3">
          <div>
            <p class="text-sm font-bold text-slate-800">{{ guide?.nomComplet || 'Non assigné' }}</p>
    ...
    <div v-if="sosActif" class="rounded-3xl border-2 border-rose-300 bg-rose-50 p-6 text-center">
      ...
      <button @click="router.push('/pole-urgence-pelerin')" class="mt-3 text-sm font-bold text-rose-700 underline">Voir le détail dans Pôle d'Urgence SOS</button>
    </div>
