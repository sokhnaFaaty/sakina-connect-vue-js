...
<template>
  <section>
    ...
    <div class="mb-6 flex flex-col gap-3 rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between dark:border-slate-700 dark:bg-slate-800">
      <div class="flex flex-wrap gap-2">
        <button
          ...
          :class="activeTab === key ? 'bg-[#333D2A] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600'"
          @click="activeTab = key"
        >
      ...
      <div class="relative sm:w-72">
        ...
        <input
          ...
          class="w-full rounded-2xl border border-slate-200 bg-white py-2.5 pl-11 pr-4 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
        />
      </div>
    </div>
    ...
    <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div v-if="itemsFiltres.length" class="grid gap-3">
        <div
          ...
          :key="item.id"
          class="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-700/50"
        >
          <div>
            <p class="font-bold text-slate-800 dark:text-slate-100">{{ ongletActif.nom(item) }}</p>
            <p class="text-xs text-slate-500 dark:text-slate-400">{{ ongletActif.sous(item) }}</p>
          </div>
      ...
      <div v-else class="flex flex-col items-center gap-2 py-10 text-center text-sm text-slate-400 dark:text-slate-500">
