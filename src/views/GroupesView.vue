      ...
      <div class="overflow-x-auto">
        <table class="min-w-full border-collapse">
          <thead class="bg-slate-50">
            <tr>
              <th class="px-5 py-4 text-left text-xs font-black uppercase text-slate-500">ID</th>
              <th class="px-5 py-4 text-left text-xs font-black uppercase text-slate-500">Nom du groupe</th>
              <th class="px-5 py-4 text-left text-xs font-black uppercase text-slate-500">Guide responsable</th>
              <th class="px-5 py-4 text-left text-xs font-black uppercase text-slate-500">Nombre de pèlerins</th>
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="g in groupes" :key="g.id" class="transition hover:bg-slate-50">
              <td class="px-5 py-4 text-xs font-bold text-slate-400">{{ g.id.slice(0, 6).toUpperCase() }}</td>
              <td class="px-5 py-4"><strong class="font-bold text-slate-950">{{ g.nom }}</strong></td>
              <td class="px-5 py-4 text-sm text-slate-700">{{ guideNom(g) }}</td>
              <td class="px-5 py-4 text-sm text-slate-700">{{ nbPelerinsParGroupe[g.id] || 0 }} pèlerins</td>
              <td class="px-5 py-4">
                <div class="flex items-center gap-3 text-base">
                  <button @click="ouvrirDetail(g)" class="text-slate-500 hover:text-slate-800" title="Voir">
      ...
      >
        <div class="flex-1 p-5">
          <div class="mb-2 flex items-start justify-between gap-2">
            ...
            <span class="rounded-md bg-[#F2F2DE] px-2 py-0.5 text-xs font-bold text-[#333D2A]">{{ g.id.slice(0, 6).toUpperCase() }}</span>
          </div>
