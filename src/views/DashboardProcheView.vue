    ...
    <template v-else>
      ...
      <div class="grid gap-4 lg:grid-cols-3">
        <article class="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p class="mt-1 text-xs text-slate-500">ID : {{ pelerin.id.slice(0, 6).toUpperCase() }} &nbsp; Passeport : {{ pelerin.numeroPasseport }}</p>
      ...
      <article v-if="sosActif" class="mt-6 rounded-[2rem] border border-rose-200 bg-rose-50 p-6">
        ...
        <div class="mt-4 grid gap-4 rounded-2xl bg-white p-4 sm:grid-cols-3">
          <div>
            ...
            <p class="font-bold text-slate-800">{{ String(sosActif.latitude ?? '-') }}</p>
          </div>
          <div>
            <p class="text-xs font-extrabold uppercase tracking-widest text-slate-400">Longitude</p>
            <p class="font-bold text-slate-800">{{ String(sosActif.longitude ?? '-') }}</p>
          </div>
          <div>
            <p class="text-xs font-extrabold uppercase tracking-widest text-slate-400">Heure</p>
            <p class="font-bold text-slate-800">{{ heureSos }}</p>
          </div>
        </div>
      ...
      <div class="grid gap-3 sm:grid-cols-3">
        <div v-for="v in validations" :key="v.label" class="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm">
          ...
          <span v-if="v.ok" class="inline-flex items-center gap-1 font-bold text-emerald-600"><i class="fa-solid fa-circle-check"></i> Valide</span>
