...
<template>
  <section>
        ...
        <div class="flex-1 p-5">
          <div class="mb-3 flex items-start justify-between gap-2">
            <div class="flex items-center gap-3">
              ...
              />
              <div v-else class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#F2F2DE] text-[#333D2A] dark:bg-slate-700/50 dark:text-[#BC7B3B]">
                <i class="fa-solid fa-user-tie"></i>
              </div>
              <div>
                <h3 class="font-black text-slate-950 dark:text-slate-100">{{ nomGuide(guide) }}</h3>
                <span class="mt-1 inline-block rounded-md bg-[#F2F2DE] px-2 py-0.5 text-xs font-bold text-[#333D2A] dark:bg-slate-700/50 dark:text-[#BC7B3B]">{{ guide.id.slice(0, 6).toUpperCase() }}</span>
              </div>
            </div>
            <span
              class="shrink-0 whitespace-nowrap rounded-full px-2.5 py-0.5 text-xs font-black"
                    :class="guide.disponibilite ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'"
            >{{ guide.disponibilite ? 'En Service' : 'Indisponible' }}</span>
          </div>

          <div class="mt-4 grid gap-2 text-sm text-slate-600 dark:text-slate-300">
            <p class="flex items-center gap-2"><i class="fa-solid fa-phone w-4 text-[#333D2A] dark:text-[#BC7B3B]"></i> {{ utilisateurMap[guide.utilisateurId]?.telephone || '-' }}</p>
            <p class="flex items-center gap-2"><i class="fa-solid fa-envelope w-4 text-[#333D2A] dark:text-[#BC7B3B]"></i> {{ utilisateurMap[guide.utilisateurId]?.email || '-' }}</p>
          </div>
        </div>

        <div class="flex items-center justify-between gap-2 bg-[#F2F2DE]/70 px-5 py-3 dark:bg-slate-700/50">
          <div class="min-w-0">
            <p class="text-[10px] font-extrabold uppercase tracking-widest text-slate-500 dark:text-slate-400">Groupe assigné</p>
            <p class="truncate text-sm font-bold text-slate-800 dark:text-slate-100">{{ groupesLabel(guide) }}</p>
          </div>
    ...
    <div v-else class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div
        v-if="guidesFiltres.length === 0"
        class="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-5 py-10 text-center text-sm font-semibold text-slate-500 dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-400"
      >
        Aucun guide ne correspond à votre recherche.
      </div>
          ...
          <table class="min-w-full border-collapse">
            <thead class="bg-slate-50 dark:bg-slate-900">
              <tr>
                ...
                <th class="whitespace-nowrap px-5 py-4 text-left text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Téléphone</th>
                <th class="whitespace-nowrap px-5 py-4 text-left text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Email</th>
                <th class="whitespace-nowrap px-5 py-4 text-left text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Disponibilité</th>
                <th class="whitespace-nowrap px-5 py-4 text-left text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Groupes assignés</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
              <tr v-for="guide in guidesFiltres" :key="guide.id" class="transition hover:bg-slate-50 dark:hover:bg-slate-700/50">
                <td class="border-t border-slate-100 px-5 py-4 align-middle text-sm text-slate-700 dark:border-slate-700 dark:text-slate-300">
                  ...
                  />
                  <div v-else class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#F2F2DE] text-[#333D2A] dark:bg-slate-700/50 dark:text-[#BC7B3B]">
                    <i class="fa-solid fa-user-tie"></i>
                  </div>
                </td>
                <td class="border-t border-slate-100 px-5 py-4 align-middle text-sm text-slate-700 dark:border-slate-700 dark:text-slate-300">
                  <strong class="font-bold text-slate-950 dark:text-slate-100">{{ utilisateurMap[guide.utilisateurId]?.nomComplet || '-' }}</strong>
                </td>
                <td class="border-t border-slate-100 px-5 py-4 align-middle text-sm text-slate-700 dark:border-slate-700 dark:text-slate-300">{{ utilisateurMap[guide.utilisateurId]?.telephone || '-' }}</td>
                <td class="border-t border-slate-100 px-5 py-4 align-middle text-sm text-slate-700 dark:border-slate-700 dark:text-slate-300">{{ utilisateurMap[guide.utilisateurId]?.email || '-' }}</td>
                <td class="border-t border-slate-100 px-5 py-4 align-middle text-sm text-slate-700 dark:border-slate-700 dark:text-slate-300">
                  <span
              ...
              :class="guide.disponibilite ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'"
                ...
                <td class="border-t border-slate-100 px-5 py-4 align-middle text-sm text-slate-700 dark:border-slate-700 dark:text-slate-300">{{ groupesLabel(guide) }}</td>
                <td class="border-t border-slate-100 px-5 py-4 align-middle text-sm text-slate-700 dark:border-slate-700 dark:text-slate-300">
        ...
        <div class="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl dark:bg-slate-800" @click.stop>
          <div class="mb-5 flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <h2 class="text-xl font-black tracking-tight text-slate-950 dark:text-slate-100">Compte guide créé</h2>
            </div>
            <button type="button" @click="fermerCompteCree" class="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 dark:text-slate-500 dark:hover:bg-slate-700 dark:hover:text-slate-100" aria-label="Fermer">
          ...
          <div class="mt-3 flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-600 dark:bg-slate-700/50">
            <span class="text-lg font-black tracking-widest text-slate-950 dark:text-slate-100">{{ compteCree.motDePasse }}</span>
