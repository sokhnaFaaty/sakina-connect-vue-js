...
<template>
  <div>
    ...
    <template v-if="sosActifs.length">
      <div v-for="s in items" :key="s.id" class="mb-3 rounded-2xl border-l-4 border-rose-500 bg-rose-50 p-4">
        <div class="mb-2 flex items-center justify-between">
          <span class="rounded-full bg-rose-600 px-2.5 py-0.5 text-xs font-black text-white">{{ s.id.slice(0, 6).toUpperCase() }}</span>
          <span class="text-xs text-slate-400"><i class="fa-regular fa-clock"></i> {{ tempsEcoule(s.dateHeure) }}</span>
        </div>
        <p class="font-black text-slate-900">{{ resolveNom(s.pelerinId) }}</p>
        ...
        <p v-if="s.commentaire" class="mt-2 text-sm italic text-slate-600">"{{ s.commentaire }}"</p>
        <button
          type="button"
