...
// Port fidèle de openPelerinDetail (js/pages/pelerinsPage.js) — modale de détail d'un pèlerin
const props = defineProps({
  pelerin: { type: Object, required: true },
  utilisateurMap: { type: Object, required: true },
  groupeMap: { type: Object, default: () => ({}) },
...
onMounted(async () => {
  ...
  } catch { /* la liste des proches reste vide */ }
})

const utilisateur = computed(() => props.utilisateurMap[props.pelerin.utilisateurId])
const groupe = computed(() => props.groupeMap[props.pelerin.groupeId])
const hotelMecque = computed(() =>
  groupe.value ? props.hotels.find((h) => h.id === groupe.value.hotelMecqueId)?.nom || '-' : '-'
)
const hotelMedine = computed(() =>
  groupe.value ? props.hotels.find((h) => h.id === groupe.value.hotelMedineId)?.nom || '-' : '-'
)
const guideNom = computed(() => {
  const guide = groupe.value ? props.guides.find((g) => g.id === groupe.value.guideId) : null
...
const procheUtilisateur = computed(() =>
  procheAssocie.value ? props.utilisateurMap[procheAssocie.value.utilisateurId] : null
)
</script>

<template>
  <Teleport to="body">
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" @click="emit('close')">
        ...
        <div class="-m-6 mb-0 flex items-start gap-4 border-b border-slate-100 p-6 pb-5">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <span class="rounded-full bg-[#F2F2DE] px-2 py-0.5 text-xs font-bold text-[#333D2A]">{{ String(pelerin.id).slice(0, 5).toUpperCase() }}</span>
            </div>
